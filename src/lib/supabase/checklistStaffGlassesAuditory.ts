...
export async function getNextPdfNumber(datePrefix: string): Promise<string> {
  try {
    ...
    const { data: files, error } = await supabase.storage
      .from('checklist-staff-glasses-auditory')
      .list('', {
        limit: 1000,
    ...
    // Pattern: YYYY-MONTHNAME-DD-Staff-Glasses-Auditory-Control-NN.pdf
    const pattern = new RegExp(`^${datePrefix}-Staff-Glasses-Auditory-Control-(\\d+)\\.pdf$`)
...
/**
 * Uploads a PDF file to Supabase Storage bucket 'checklist-staff-glasses-auditory'
...
export async function uploadChecklistPDF(
  ...
  try {
    ...
    console.log('Uploading PDF to checklist-staff-glasses-auditory:', {
      filename,
      fileSize: file.size,
...
/**
 * Interface for checklist staff glasses auditory data
 */
export interface ChecklistStaffGlassesAuditoryData {
  date_string: string
  monitor_name: string
...
/**
 * Inserts a record into the checklist_staff_glasses_auditory table
 * @param data - The checklist data to insert
 * @returns The inserted row data
 * @throws Error if insert fails
 */
export async function insertChecklistStaffGlassesAuditory(
  data: ChecklistStaffGlassesAuditoryData
): Promise<any> {
  try {
    // Check authentication
    const { data: { session } } = await supabase.auth.getSession()
    ...
    console.log('Inserting checklist_staff_glasses_auditory record:', {
      date_string: data.date_string,
      monitor_name: data.monitor_name,
      persons_count: data.persons?.length || 0,
      no_findings: data.no_findings,
    ...
    const { data: insertedData, error } = await supabase
      .from('checklist_staff_glasses_auditory')
      .insert([insertData])
      .select()

    if (error) {
      console.error('Error inserting checklist_staff_glasses_auditory record:', {
        message: error.message,
        code: error.code,
    ...
    console.log('Checklist_staff_glasses_auditory record inserted successfully:', insertedData[0])
    return insertedData[0]
  } catch (error) {
    console.error('Error in insertChecklistStaffGlassesAuditory:', error instanceof Error ? {
      message: error.message,
      stack: error.stack,
      name: error.name
    } : error)
...
/**
 * Fetches checklist staff glasses auditory records with optional date filtering
 ...
 */
export async function fetchChecklistStaffGlassesAuditoryData(
  startDate?: string,
  endDate?: string
): Promise<any[]> {
  try {
    ...
    if (error) {
      console.error('Error fetching checklist_staff_glasses_auditory records:', {
        message: error.message,
        code: error.code,
        details: error.details,
        hint: error.hint,
    ...
    console.error('Error in fetchChecklistStaffGlassesAuditoryData:', error instanceof Error ? {
      message: error.message,
      stack: error.stack,
      name: error.name
    } : error)
