...
/**
 * Uploads a PDF file to Supabase Storage bucket 'checklist-foreign-material'
...
export async function uploadChecklistPDF(
  ...
  try {
    ...
    console.log('Uploading PDF to checklist-foreign-material:', {
      filename,
      fileSize: file.size,
    ...
    const { data, error } = await supabase.storage
      .from('checklist-foreign-material')
...
/**
 * Interface for checklist foreign material data
 */
export interface ChecklistForeignMaterialData {
  date_string: string
  brand: string
  product: string
  shift: string
...
/**
 * Inserts a record into the checklist_foreign_material table
 * @param data - The checklist data to insert
 * @returns The inserted row data
 * @throws Error if insert fails
 */
export async function insertChecklistForeignMaterial(
  data: ChecklistForeignMaterialData
): Promise<any> {
  try {
    // Check authentication
    const { data: { session } } = await supabase.auth.getSession()
    ...
    console.log('Inserting checklist_foreign_material record:', {
    ...
    const { data: insertedData, error } = await supabase
      .from('checklist_foreign_material')
      .insert([insertData])
      .select()

    if (error) {
      console.error('Error inserting checklist_foreign_material record:', {
        message: error.message,
        code: error.code,
    ...
    console.log('Checklist_foreign_material record inserted successfully:', insertedData[0])
    return insertedData[0]
  } catch (error) {
    console.error('Error in insertChecklistForeignMaterial:', error instanceof Error ? {
      message: error.message,
      stack: error.stack,
      name: error.name
    } : error)
...
/**
 * Fetches checklist foreign material records with optional date filtering
 ...
 */
export async function fetchChecklistForeignMaterialData(
  startDate?: string,
  endDate?: string
): Promise<any[]> {
  try {
    ...
    if (error) {
      console.error('Error fetching checklist_foreign_material records:', {
        message: error.message,
        code: error.code,
        details: error.details,
        hint: error.hint,
    ...
    console.error('Error in fetchChecklistForeignMaterialData:', error instanceof Error ? {
      message: error.message,
      stack: error.stack,
      name: error.name
    } : error)
